...
static void bl_write_progress(struct bl_eth_device *dev)
{
    ...
    //printk("xfer->cur_skb:%px empty:%d\n", xfer->cur_skb, skb_queue_empty(&dev->tx_sk_list));
	...
	//printk("len:%ld\n", len);
	    ...
	    //printk("XRAM_NET_HEADER:%x skb len:%d\n", 
			    //*((uint32_t *)(skb->data)), skb->len);
#ifdef BUSY_DROP_PKT
	    int retry_cnt = 0;
	    int buffer_ok = 0;
	    while(retry_cnt < 5)
...
static void bl_main_wq_handler(struct work_struct *work)
{
    struct bl_eth_device *dev = container_of(work, struct bl_eth_device, main_work);

    //printk("bl_main_wq_handler\n");
    while (1) {
        u8 events;
...
static struct net_device *init_netdev(struct bl_eth_device *dev, size_t idx)
{
    ...
    netdev->dev_addr[0] = 0x18;
    netdev->dev_addr[1] = 0xb9;
    netdev->dev_addr[2] = 0x05;
    netdev->dev_addr[3] = 0x88;
    netdev->dev_addr[4] = 0x88;
    netdev->dev_addr[5] = 0x88;
...
{
    BL_MASK_IRQ(dev, BL_XRAM_RING_EVENT_MASK);
...
static int bl_xram_eth_init(struct platform_device *pdev)
{
    ...
    INIT_WORK(&dev->main_work, bl_main_wq_handler);
...
static const struct of_device_id bflb_ipc_match[] = {
    { .compatible = "bflb-ipc,ipc0", }, //bflb-ipc,ipc0
    {},
};
